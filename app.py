...
# ==========================================
@st.dialog("👤 進捗詳細（閲覧専用）")
def show_user_progress_detail(n_user):
    """選択された新人の各進捗の詳細情報を表示するダイアログ（修正不可）"""
    target_uid = str(n_user['id'])
    st.subheader(f"{n_user['name']} さんの進捗詳細")

    p_path = os.path.join(USERS_DIR, target_uid, "my_progress.csv")

    if not os.path.exists(p_path) or not os.path.exists(TASK_CSV):
        st.info("データが見つかりません。")
        return

    df_user_p = pd.read_csv(p_path, encoding="utf_8_sig")
    df_tasks_master = pd.read_csv(TASK_CSV, encoding="utf_8_sig")

    categories = ["調剤室業務", "注射室業務", "DI室", "製剤室", "ミキシング", "麻薬"]
    ...
    for idx, cat_name in enumerate(categories):
        with tabs[idx]:
            ...
            if m_sub.empty:
                st.caption("対象項目がありません。")
                continue

            merged = pd.merge(
                m_sub[['項目']],
                df_user_p[df_user_p['カテゴリ'] == cat_name][['項目', '習得度']],
                on='項目',
                how='left'
            )
            merged['習得度'] = merged['習得度'].fillna(1).infer_objects().astype(int)

            # 評価ラベルへのマッピング（表示用）
            level_map = {1: "1: 未経験", 2: "2: 見学済", 3: "3: 実施（指導有）", 4: "4: 実施（見守り）", 5: "5: 独り立ち"}
            merged['習得度ステータス'] = merged['習得度'].map(level_map)

            # 修正不可の閲覧専用テーブルで表示
            st.dataframe(
                merged[['項目', '習得度ステータス']],
                column_config={
                    "項目": st.column_config.TextColumn("チェック項目"),
                    "習得度ステータス": st.column_config.TextColumn("現在の習得レベル")
                },
                hide_index=True,
                width='stretch'
            )
...
def show_main_menu():
    ...
    with h_col4:
        ...
        if role == "教育係":
            with inner_cols[col_idx]:
                if st.button("👥 プリセプター用メニュー", key="h_mentor", width='stretch'):
        ...
        with inner_cols[col_idx]:
            if st.button("📊 履歴", key="h_history", type="secondary", width='stretch'):
                st.session_state['page'] = 'review'
                st.rerun()
        col_idx += 1

        with inner_cols[col_idx]:
            if st.button("🚪 終了", key="h_logout", type="secondary", width='stretch'):
                st.session_state.clear()
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if not os.path.exists(LOGIN_CSV):
        st.warning(f"⚠️ ファイルが見つかりません: {LOGIN_CSV}")
    elif not os.path.exists(TASK_CSV):
        st.warning(f"⚠️ ファイルが見つかりません: {TASK_CSV}")
    else:
        df_users = pd.read_csv(LOGIN_CSV, encoding="utf_8_sig")

        # ロール判断（「新人」が含まれるユーザーを抽出）
        newcomers = df_users[df_users['role'].astype(str).str.contains("新人")]

        if newcomers.empty:
            st.info("ℹ️ login_data.csv 内に role が『新人』『新人薬剤師』となっているユーザーが見つかりません。")
        else:
            df_tasks_master = pd.read_csv(TASK_CSV, encoding="utf_8_sig")
            summary_list = []

            for _, n_user in newcomers.iterrows():
                target_uid = str(n_user['id'])
                p_path = os.path.join(USERS_DIR, target_uid, "my_progress.csv")

                row_data = {"新人氏名": n_user['name']}

                df_user_p = pd.read_csv(p_path, encoding="utf_8_sig") if os.path.exists(p_path) else pd.DataFrame(
                    columns=['カテゴリ', '項目', '習得度']
                )

                for cat_name in ["調剤室業務", "注射室業務", "DI室", "製剤室", "ミキシング", "麻薬"]:
                    m_sub = df_tasks_master[df_tasks_master['カテゴリ'] == cat_name]
            ...
            if summary_list:
                ...
                with st.container(border=True):
                    st.markdown(
                        "##### 📈 新人薬剤師育成進捗一覧 <span style='font-size: 0.8rem; color: #64748b;'>(※行をクリックすると詳細を確認できます)</span>",
                        unsafe_allow_html=True)
                    ...
                    event = st.dataframe(
                        df_summary,
                        column_config={
                            "新人氏名": st.column_config.TextColumn("氏名"),
                            "調剤室業務 進捗": st.column_config.ProgressColumn("調剤室", format="%d%%", min_value=0,
                                                                               max_value=100),
                            "注射室業務 進捗": st.column_config.ProgressColumn("注射室", format="%d%%", min_value=0,
                                                                               max_value=100),
                            "DI室 進捗": st.column_config.ProgressColumn("DI室", format="%d%%", min_value=0,
                                                                         max_value=100),
                            "製剤室 進捗": st.column_config.ProgressColumn("製剤室", format="%d%%", min_value=0,
                                                                           max_value=100),
                            "ミキシング 進捗": st.column_config.ProgressColumn("ミキシング", format="%d%%", min_value=0,
                                                                               max_value=100),
                            "麻薬 進捗": st.column_config.ProgressColumn("麻薬", format="%d%%", min_value=0,
                                                                         max_value=100),
                        },
                        hide_index=True,
                        width='stretch',
                    ...
                    if event and event.selection and event.selection.get("rows"):
                        ...
                        selected_user = newcomers.iloc[selected_idx]
                        show_user_progress_detail(selected_user)

    # --- 3. メインメニュー（カード） ---
    st.markdown(
        "<h3 style='text-align: center; margin-top: 20px; margin-bottom: 25px; color: #475569;'>MENU</h3>",
        unsafe_allow_html=True
    )
    ...
    menu_items = [
        ...
        {"title": "⚙ マスター管理", "id": "master_editor", "col": m_col2}
    ]

    for item in menu_items:
        with item['col']:
            if st.button(item['title'], key=f"menu_{item['id']}", width='stretch'):
                st.session_state['page'] = item['id']
                if item['id'] == "master_editor":
...
def show_checklist_menu():
    ...
    with col1:
        ...
        # 3. DI室（設計中）
        if st.button("ℹ️ DI室 (設計中)", use_container_width=True):
            st.session_state['current_task_view'] = 'DI室'
            st.session_state['page'] = 'checklist_detail'
            st.rerun()

        # 5. ミキシングルーム
        if st.button("🧪 ミキシングルーム", use_container_width=True):
            st.session_state['current_task_view'] = 'ミキシング'
            st.session_state['page'] = 'checklist_detail'
            st.rerun()
    ...
    with col2:
        ...
        # 4. 製剤室（設計中）
        if st.button("⚗️ 製剤室 (設計中)", use_container_width=True):
            st.session_state['current_task_view'] = '製剤室'
            st.session_state['page'] = 'checklist_detail'
            st.rerun()

        # 6. 麻薬
        if st.button("🧪 麻薬", use_container_width=True):
            st.session_state['current_task_view'] = '麻薬'
            st.session_state['page'] = 'checklist_detail'
            st.rerun()
    ...
    if summary_list:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "DI室 進捗": st.column_config.ProgressColumn("DI室", format="%d%%", min_value=0, max_value=100),
                "製剤室 進捗": st.column_config.ProgressColumn("製剤室", format="%d%%", min_value=0, max_value=100),
                "ミキシング 進捗": st.column_config.ProgressColumn("ミキシング", format="%d%%", min_value=0, max_value=100),
                "麻薬 進捗": st.column_config.ProgressColumn("麻薬", format="%d%%", min_value=0, max_value=100),
            },
...
def render_individual_detail(user, df_tasks_master):
    ...
    with t2:
        ...
        c1, c2 = st.columns(2)
        # インデックスを2で割った余り（0か1）を使って、c1とc2を交互に選択する
        cols = [c1, c2]

        for i, cat in enumerate(["調剤室業務", "注射室業務", "DI室", "製剤室", "ミキシング", "麻薬"]):
            with cols[i % 2]:  # ← ここを修正
                st.markdown(f"**【{cat}】**")
                ...
                st.dataframe(display_df, hide_index=True, use_container_width=True)  # width='stretch' の警告対策も兼ねて修正
...
def render_questions_form_editor():
    ...
    if st.button(btn_label, type="primary", use_container_width=True):
        if not question_text:
            st.error("問題文は必須です。")
        elif ref_mode == "新しく資料を登録" and not final_url:
            st.error("URLを入力してください。")
        else:
            try:
                # 現在の時刻とユーザー名を取得
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M")
                current_user = st.session_state.user.get('name', 'admin')

                # --- A. 資料ライブラリへの追加 (URL登録のみ) ---
                if ref_mode == "新しく資料を登録" and final_ref_title:
                    ...
                    new_lib_row = {
                        "大カテゴリー": major,
                        "小カテゴリー": minor,
                        "タイトル": final_ref_title,
                        "タイプ": "URL",
                        "ファイル名": "",
                        "URL": final_url if 'final_url' in locals() else "",
                        "登録者": current_user,
                        "登録日": current_time
                    }
...
def main():
    ...
    elif current_page == 'master_editor':
        # フォーム編集モードならエディタを、それ以外（"list"など）なら一覧画面を表示する
        if st.session_state.get("master_mode") == "form":
            render_questions_form_editor()
        else:
            render_master_editor()  # ← ここを追加！
